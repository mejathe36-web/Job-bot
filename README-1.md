...
International jobs load automatically from ReliefWeb's free public feed (engineering-related openings).

**One-time setup for the international feed:** ReliefWeb now requires a pre-approved "appname" for API access. Request one for free at https://reliefweb.int (search "ReliefWeb API appname request form") — approval usually takes about a business day. Once you have it, open `index.html`, find this line near the bottom:

```
const RELIEFWEB_APPNAME = 'jobboard-demo';
```

Replace `'jobboard-demo'` with your approved appname, save, and re-upload the file to GitHub. Until you do this, the international tab may show no results or an error message.
